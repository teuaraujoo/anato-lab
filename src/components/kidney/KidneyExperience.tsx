...
const KidneyScene = dynamic(() => import("./KidneyScene"), {
  ssr: false,
  loading: () => (
...
function StructurePanel({
  ...
  const id = useKidneyStore((state) => state.selectedId);
  const isolated = useKidneyStore((state) => state.isolatedId);
  const select = useKidneyStore((state) => state.select);
  const selected = id ? kidneyContentById[id] : null;
  const matches = searchKidneyStructures(query);
  return (
    <aside
      id="estruturas"
      className="study-panel"
      aria-label="Estruturas do rim"
      hidden={!isOpen}
    >
      <div className="panel-heading">
        <h2>Explore as estruturas</h2>
        <span className="panel-count">{kidneyContent.length}</span>
        <button
        ...
        <div className="structure-detail" aria-live="polite" aria-atomic="true">
          {selected ? (
              ...
              <div className="related-list">
                {selected.related.map((related) => (
                  <button key={related} onClick={() => select(related)}>
                    {kidneyContentById[related].name}
            ...
            <div className="detail-intro">
              ...
              <h3>Como o rim participa do equilíbrio do corpo?</h3>
              <p>
                Selecione uma estrutura no modelo ou na lista para entender como
                ela se conecta ao funcionamento do rim.
              </p>
              <button
                className="text-button"
                onClick={() => select("renal-cortex")}
              >
                Começar pelo córtex{" "}
...
function ViewerToolbar() {
  const selected = useKidneyStore((state) => state.selectedId);
  const isolated = useKidneyStore((state) => state.isolatedId);
  const toggleIsolation = useKidneyStore((state) => state.toggleIsolation);
  const focus = useKidneyStore((state) => state.focusSelection);
  const reset = useKidneyStore((state) => state.reset);
  const exploded = useKidneyStore((state) => state.exploded);
  const toggleExploded = useKidneyStore((state) => state.toggleExploded);
  return (
    <div
    ...
    >
      <button
        ...
        onClick={reset}
        title="Restaurar câmera e mostrar o rim inteiro"
      >
...
export function KidneyExperience() {
  ...
  const selectedId = useKidneyStore((state) => state.selectedId);
  const hoveredId = useKidneyStore((state) => state.hoveredId);
  const isolated = useKidneyStore((state) => state.isolatedId);
  const exploded = useKidneyStore((state) => state.exploded);
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "Escape") useKidneyStore.getState().reset();
    };
    window.addEventListener("keydown", onKey);
    return () => {
      window.removeEventListener("keydown", onKey);
      useKidneyStore.getState().reset();
    };
  }, []);
  return (
    <main ref={rootRef} className="atlas-page kidney-page">
      <a
        className="skip-link"
        href="#estruturas"
        onClick={(event) => {
      ...
      <div className="page-intro" id="explorador">
        ...
        >
          ...
          <span className="trigger-count">{kidneyContent.length}</span>
        </button>
      </div>
            ...
            style={
              {
                "--structure-color": current
                  ? kidneyContentById[current].color
                  : "#69dbd7",
              } as CSSProperties
            }
          >
            <div className="stage-header">
              <span className="specimen-label">SISTEMA URINÁRIO</span>
            ...
            <div className="stage-caption">
              ...
              {current
                ? kidneyContentById[current].name
